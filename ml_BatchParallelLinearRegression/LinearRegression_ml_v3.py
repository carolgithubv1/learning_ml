...
epsilon_list                 = []
epsilon_max_index            = np.zeros((N_batches, N_xy_perBatch, 1))
epsilon_max_index_list       = []
epsilon_sortedTopN_small2big = np.zeros((N_batches, N_top))
epsilon_mean                 = np.zoros((N_batches, 1))
epsilon_mean_list            = []
...
for k in range(N_epoch):
    ...
    for i in range(N_iter):
	    ...
	    for j in range(N_batches):
		    ...
		    for i in range(N_xy_perBatch):
		        ...
		        d[ii][i]       = normalDistance(a_b_c[j][i], x_y[j][i])
    ...
    for jj in range(N_batches): 
    	for ii in range(N_xy_perBatch): 
    		# derivative and based input(x,y) to update (a,b,c)
    		df_c2 = df_cost2(a_b_c_c2, x_y[jj][ii])
    		a_b_c_is_c2[jj][ii] = update_coef(a_b_c_c2, df_c2)
    		# epsilon of cost function 2
    		epsilon[jj][ii]     = epsilon_cost2(a_b_c_c2, x_y[jj][ii])
    		epsilon_list.append(float(epsilon[jj][ii]))
    		# update a_b_c_c2
    		a_b_c_c2            = a_b_c_is_c2[jj][ii]
    	# the index of minimum of epsilon per batch
    	epsilon_min_index[jj]   = np.argmin(np.asarray(epsilon_list))
    	epsilon_min_index_list.append(int(epsilon_max_index[jj]))
    ...
    # index of the miimum of epsilon of all 
    epsilon_min_index_allBatches = np.argmin(np.asarray(epsion_mean_list))
